...
if ($path === "$prefix/stats/summary" && $method === 'GET') {
  ...
  // Für Player: Nur Statistiken von zugeordneten Discs
  $query = "
    SELECT 
      COUNT(*) AS count,
  ...
  // Für Player: Nur Würfe von zugeordneten Discs
  if ($playerId !== null && $playerId !== '' && ($scheibeId === null || $scheibeId === '')) {
    $query .= " AND scheibe_id IN (SELECT disc_id FROM player_discs WHERE player_id = :player_id)";
